...
#define CUSTOMER_MAC_FILE "/data/rand_mac.txt" /*"/data/wifimac.txt"*/

// #define GPIO_WIFI_SHUTDOWN	90
...
int wlan_device_power(int on)
{
	...
	/* enable SDIO clock */
		...
		mdelay(10);
		gpio_direction_output(sprd_3rdparty_gpio_wifi_power, 1);
		mdelay(200);
	}
	else {
	/* disale SDIO clock */
...
static int __init wlan_device_init(void)
{
	...
	wlan_ldo_enable();
	wlan_clk_init();
	gpio_request(sprd_3rdparty_gpio_wifi_irq, "oob_irq");
    printk("MEUI: after gpio_request(sprd_3rdparty_gpio_wifi_irq=%d)\n", sprd_3rdparty_gpio_wifi_irq);
    ...
    printk("MEUI: after gpio_request(sprd_3rdparty_gpio_wifi_power=%d)\n", sprd_3rdparty_gpio_wifi_power);
	gpio_direction_output(sprd_3rdparty_gpio_wifi_power, 0);

    printk("MEUI: before platform_device_register(&sprd_wlan_device), wlan_resources[1].start=%du, .end=%du\n", wlan_resources[1].start, wlan_resources[1].end);
